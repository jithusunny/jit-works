 ...
 */
const HERO_H = 'clamp(300px, calc(10dvh + 60vw), 74dvh)';
...
export default function Showcase() {
  ...
  // Desktop wrap height tracks 78vw so the scene scales down with the window (both people
  // stay in frame); a soft bottom fade on the wrap guarantees the photo dissolves instead of
  // ending in a hard edge when the shortened wrap ends before the radial mask does.
  const WRAP_FADE = 'linear-gradient(to bottom, #000 76%, transparent 98%)';
  const heroPhotoWrapStyle = isMobile
    ? sx({ position: 'absolute', left: '-18vw', top: '-9dvh', width: '138vw', height: '64dvh', pointerEvents: 'none' })
    : sx({ position: 'absolute', left: '-4.2vw', top: '-8.3dvh', width: '81.3vw', height: 'min(100dvh, 78vw)', pointerEvents: 'none', WebkitMaskImage: WRAP_FADE, maskImage: WRAP_FADE });
  ...
  // The design's original dissolve masks. They reach full transparency inside the photo
  // element, so the image never shows a hard rectangular edge.
...
function ContactOverlay({
  ...
  const optionStyle = sx({ display: 'grid', gridTemplateColumns: '42px minmax(0,1fr) auto', alignItems: 'center', gap: '13px', padding: isMobile ? '13px' : '14px', border: '1px solid #E4DFCF', borderRadius: '16px', color: '#20211B', textDecoration: 'none', background: '#FFFDF7' });
  const iconStyle = sx({ display: 'grid', placeItems: 'center', width: '42px', height: '42px', borderRadius: '13px' });
  const channelNameStyle = sx({ display: 'block', fontWeight: 700, lineHeight: 1.2 });
  const channelHintStyle = sx({ display: 'block', marginTop: '3px', color: '#8B8977', fontSize: '12.5px', lineHeight: 1.35 });
  ...
  return (
    ...
    >
      ...
      <div style={sx({ display: 'grid', gap: '10px' })}>
        <a class="contact-channel" href={WHATSAPP_URL} target="_blank" rel="noopener" style={optionStyle}>
          <span class="contact-channel__icon contact-channel__icon--whatsapp" aria-hidden="true" style={iconStyle}>
          ...
          <span>
            <span style={channelNameStyle}>WhatsApp</span>
            <span style={channelHintStyle}>Quick questions or a chat</span>
          </span>
          <span class="contact-channel__arrow" aria-hidden="true" style={sx({ color: '#2E5E43', fontSize: '21px' })}>↗</span>
        </a>
        <a class="contact-channel" href={EMAIL_URL} style={optionStyle}>
          <span class="contact-channel__icon contact-channel__icon--email" aria-hidden="true" style={iconStyle}>
            <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" focusable="false">
              <rect x="3" y="5" width="18" height="14" rx="2.5" />
              <path d="m4.5 7 7.5 6 7.5-6" />
            </svg>
          </span>
          <span>
            <span style={channelNameStyle}>Email</span>
            <span style={channelHintStyle}>Details, files, or longer notes</span>
          </span>
          <span class="contact-channel__arrow" aria-hidden="true" style={sx({ color: '#2E5E43', fontSize: '21px' })}>↗</span>
        </a>
      </div>
